<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="checkmg.aspx.cs" Inherits="ComOpp.BackAdmin.chance.checkmg" %>
...
<body>
    <!--商机管理状态切换标签-->
    <div class="nav_tabs">
        <div class="eachTab<%=GetString("action") == "" ? " tchose" : string.Empty %>" data-link="?" data-key="index" data-state="">
            全部显示</div>
        <div class="eachTab<%=GetString("action") == "intent" ? " tchose" : string.Empty %>" data-link="?state=21&action=intent" data-key="intent" data-state="21">转出至清洗</div>
        <div class="eachTab<%=GetString("action") == "potential" ? " tchose" : string.Empty %>" data-link="?state=21&action=potential" data-key="potential" data-state="21">转出至潜客</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $(".nav_tabs .eachTab").live('click', function () {
            ...
            window.location.href = $(this).attr('data-link') + "&corpid=" + corpid + "&checkstate=1";
        });
        $("#So_Field").combobox({
            onSelect: function (option) {
                ...
                } else if ($.inArray(option.value, ["followalarm", "arrive", "state", "archivemove"]) != -1) {
                    $("span.fieldtext").hide();
                    $("span.fieldselect").show();
                    $("#keywords_select").combobox({
                        method: 'get',
        ...
        Core.Easyui.get('chancelist.aspx?corpid=' + corpid + '&checkstate=1&action=<%=GetString("action") %>'<%if(GetInt("state",0)>0){ %> + '&state=<%=GetInt("state",0) %>'<%} %>);
