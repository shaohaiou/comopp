...
{
    public partial class customermoverecordlist : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<CustomerMoveRecordInfo> list = CustomerMoveRecords.Instance.GetListByCustomerID(cid,true);
                list = list.OrderByDescending(l => l.ID).ToList();

                int total = list.Count;
                int maxpage = list.Count / rows + (list.Count % rows == 0 ? 0 : 1);

                list = list.Skip((page - 1) * rows).Take(rows).ToList<CustomerMoveRecordInfo>();
