<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="chancefollowadd.aspx.cs"
    Inherits="ComOpp.BackAdmin.chance.chancefollowadd" %>
...
<div>
    ...
    <table class="guestdata followtable" width="100%">
        <tr>
            ...
            <td>
                <%=CurrentCustomerInfo == null ? string.Empty : CurrentCustomerInfo.Name%>
            </td>
            <td class="tit">
                客户电话：
            </td>
            <td>
                <%= CurrentCustomerInfo == null ? string.Empty : CurrentCustomerInfo.Phone%>
            </td>
        </tr>
        <tr>
            <td class="tit">
            ...
            <td>
                <select class="easyui-combobox selec" style="width: 140px; height: 28px;" name="form[connectway]"
                    id="connectway">
            ...
            <td title="格式:yyyy-mm-dd hh:mm">
                <input id="followtime" class="easyui-datetimebox" name="form[followtime]" value="<%=DateTime.Now.ToString("yyyy-MM-dd HH:mm") %>"
                    data-options="showSeconds:false" title="格式:yyyy-mm-dd hh:mm" style="width: 150px">
            </td>
        </tr>
        <tr>
            <td class="tit">
            ...
            <td colspan="3">
                <textarea style="width: 430px; height: 54px;" name="form[followinfo]"></textarea>
            </td>
        </tr>
        <tr>
            <td class="tit">
            ...
            <td>
                <select class="easyui-combobox selec" style="width: 140px; height: 28px;" name="form[customerlevel]"
                    id="customerlevel">
                    <option value="">请选择追踪级别</option>
                    <asp:repeater id="rptCustomerLevel" runat="server">
                    <itemtemplate>
                        <option value="<%#Eval("ID") %>" <%# SetCustomerLevelSel(Eval("ID")) %> data-introduce="<%# SetCustomerLevelIntroduce(Eval("ID")) %>"><%#Eval("Name") %></option>
                    </itemtemplate>
                  </asp:repeater>
                </select>
            </td>
            ...
            <td>
                <select class="easyui-combobox selec" style="width: 150px; height: 28px;" name="form[uid]"
                    id="uid">
        ...
        <tr>
            <td>&nbsp;</td>
            <td colspan="3">
                <span id="txtlevelintroduce" style="color:Gray;"><%=CurrentLevelIntroduce %></span></td>
        </tr>
    </table>
    <div class="formTip fl">
    </div>
    <div class="call_save_box fr" style="margin: 0 20px 10px 0;">
        <a onclick="javascript:$('#dialog').dialog('close')" class="btn">取消关闭</a></div>
    <div class="call_save_box fr" style="margin: 0 20px 10px 0;">
        <input type="button" class="btn" value="确定提交" onclick="return Core.Easyui.Post(this);"></div>
    </form>
</div>
...
$(function(){
    $("#customerlevel").combobox({
        onSelect:function(option){
            if(option.value != "")
                $("#txtlevelintroduce").text($("#customerlevel option[value='" + option.value +"']").attr("data-introduce"));
            else
                $("#txtlevelintroduce").text("");
        }
    });
});
</script>
