...
{
    public partial class powergrouplist : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<PowerGroupInfo> list = PowerGroups.Instance.GetList(true);
                if (corpid > 0)
                ...
                list = list.Skip((page - 1) * rows).Take(rows).ToList<PowerGroupInfo>();
