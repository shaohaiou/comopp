<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="carmodelmg.aspx.cs" Inherits="ComOpp.BackAdmin.system.carmodelmg" %>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>车型管理-红旭集团销售客户管理系统V1.0</title>
...
<body>
    <div class="Tab">
        <a href="corporationmg.aspx">公司管理</a> <a href="modulemg.aspx">模块管理</a> <a href="carbrandmg.aspx">
            车辆品牌</a> <a href="carseriesmg.aspx">车系管理</a> <a href="carmodelmg.aspx" class="selected">
                车型管理</a>
    </div>
    <div>
        <div style="padding-top: 1px;">
            ...
            <div class="schlist">
                <div style="padding-top: 1px;">
                    <asp:UpdatePanel runat="server" ID="up1" style="display:inline-block;">
                        <ContentTemplate>
                            <label>
                                品牌：</label>
                            <asp:DropDownList runat="server" ID="ddlCarBrandSearch" AutoPostBack="true" OnSelectedIndexChanged="ddlCarBrandSearch_SelectedIndexChanged">
                            </asp:DropDownList>
                            <asp:DropDownList runat="server" ID="ddlCarSeriesSearch">
                            </asp:DropDownList>
                        </ContentTemplate>
                    </asp:UpdatePanel>
    ...
    <script type="text/javascript">
        ...
        { field: 'name', title: '车型', width: 300, align: 'left' },
        { field: 'seriesname', title: '车系', width: 140, align: 'left' },
        { field: 'brandname', title: '品牌', align: 'left' }
        ]);
        Core.Easyui.Form = function (o) {
            var p = { 'brandid': $("#ddlCarBrandSearch option:selected").val(),
                'seriesid': $("#ddlCarSeriesSearch option:selected").val()
            };
            Core.Easyui.load(p);
        }
        Core.Easyui.Action = function (method, o) {
            ...
            switch (method) {
                case 'add':
                    dialog('580', '300', '添加', "carmodeledit.aspx?r=" + Math.random());
                    break;
                case 'edit':
                    if (!rows.length) { $.messager.alert('提示', '请选择一条记录', 'info'); return; }
                    if (rows.length > 1) { $.messager.alert('提示', '该操作仅允许1条数据', 'info'); return; }
                    dialog('580', '300', '编辑', "carmodeledit.aspx?id=" + rows[0].id + "&r=" + Math.random());
                    break;
                case 'remove':
                    var ids = []
        ...
        $(document).ready(function () {
            Core.Easyui.get("/system/carmodellist.aspx");
        })
    </script>
    <script type="text/javascript">
        $(document).ready(function () {
