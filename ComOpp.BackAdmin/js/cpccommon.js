...
function TV_Initialize() {
    if (!cpcxDevice.loading()) {
        winWarn('400', '120', '警告', '您的系统未安装设备驱动，请先下载安装并激活设备！推荐使用Google Chrome浏览器', Sphone.driver); return;
    }
    qnviccub = document.getElementById('qnviccub');
    var version = '';
    try { version = qnviccub.GetVersionInfo().replace(/ /g, "") } catch (e) { }
    if (version != 'Jan82015') {
        winWarn('400', '120', '警告', '您的系统设备驱动版本过期，请下载安装最新设备驱动并安装！<br /><span style="color:red">安装时请关闭所有浏览器</span>！', Sphone.driver); return;
    }
    if (cpcxDevice.box["recorddir"] == '') {
        $('body').append('<div id="winDialog"></div>');
        $('#winDialog').dialog({
            title: '警告',
            width: '400',
            height: '140',
            content: '您还未设置电话录音文件储存目录，请到“个人设置”-> “个人电话设置”中配置。<br /><span style="color:red">目录路径只允许使用纯字母，请勿使用中文、空格等字符！</span>',
            modal: true,
            buttons: [{ text: '关闭提示', handler: function () { $('#winDialog').dialog('close'); } }]
        });
    }
    if (qnviccub.QNV_DevInfo(0, QNV_DEVINFO_GETCHANNELS) <= 0) {
        ...
        if (channels > 0) {
            ...
            try { cpcxDevice.box["device"] = true; Sphone.device = true; } catch (e) { }
        ...
        } else if (!qnviccub.IsAuthorized()) {
            $.messager.confirm('设备激活提醒', '设备需授权激活后方可正常使用！<br />您确定现在要激活吗？', function (r) {
                if (!r) return;
                dialog('370', '165', "设备激活", Sphone.auth + "?code=" + qnviccub.GetMachineCode() + "&r=" + Math.random()); return;
            });
            return;
        } else {
            AppendStatus("打开设备失败,请检查设备是否已经插入并安装了驱动,并且没有其它程序已经打开设备"); return;
        }
        //初始化变量
        for (i = 0; i < uMaxID; i = i + 1) {
            uPlayFileID[i] = -1; uRecordID[i] = -1;
        }
        addEvent(qnviccub, 'on_event', function (uChannelID, lType, lHandle, lResult, lParam, pData, pDataEx) { cpcxDevice.EventListener(uChannelID, lType, lHandle, lResult, pData) });
    } else {
...
function TV_StartRecordFile(filename) {
    ...
    TV_StopRecordFile(channel); //先停止录音
    uRecordID[channel] = qnviccub.QNV_RecordFile(channel, QNV_RECORD_FILE_START, vFormatID, vmask, filename);
    if (uRecordID[channel] <= 0) {
        AppendStatusEx(channel, "录音失败:" + filename);
    } else {
        AppendStatusEx(channel, "开始录音文件: id=" + uRecordID[channel] + "  " + filename);
        return true;
    }
}
/*
...
function TV_StartDial(uID, szCode) {//正常拨号必须使用 DIALTYPE_DTMF
    if (qnviccub.QNV_General(uID, QNV_GENERAL_STARTDIAL, DIALTYPE_DTMF, szCode) <= 0) {
        Core.ccPhone.Status("callfail", szCode);
        AppendStatusEx(uID, "拨号失败:" + szCode);
    } else {
        Core.ccPhone.Status("call", szCode);
...
function TV_BrowerPath() {
    ...
    //try{cpcxDevice.recorddir(vPath);}catch(e){}
    try { window.frames['riframe'].Core.SetRecordpath(vPath); } catch (e) { }
}
...
function pluginLoaded() { try { Sphone.openDeviceSucc = true; } catch (e) { } }
