<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="follow.aspx.cs" Inherits="ComOpp.BackAdmin.analyse.follow" %>
...
<body>
    <div class="Tab">
        <a href="archive.aspx?corpid=<%=GetInt("corpid") %>">销售线索分析</a> <a href="invite.aspx?corpid=<%=GetInt("corpid") %>">
            客户邀约分析</a> <a href="finality.aspx?corpid=<%=GetInt("corpid") %>">成交与战败分析</a>
        <a href="follow.aspx?corpid=<%=GetInt("corpid") %>" class="selected">客户追踪分析</a>
    </div>
    <form id="Form1" runat="server">
    <div class="nav_tabs" style="padding-left: 15px; margin-top: 5px; padding-bottom: 5px;
        border-bottom: 1px #ddd solid">
        <div class="eachTab<%if(GetString("active") == "state"){ %> tchose<%} %>" data-key="state" data-link="follow.aspx?active=state&corpid=<%=GetInt("corpid") %>">
            客户状态</div>
        <div class="eachTab<%if(GetString("active") == "customerlevel"){ %> tchose<%} %>" data-key="customerlevel" data-link="follow.aspx?active=customerlevel&corpid=<%=GetInt("corpid") %>">
            客户级别</div>
        <div class="eachTab<%if(GetString("active") == "overtime"){ %> tchose<%} %>" data-key="overtime" data-link="follow.aspx?active=overtime&corpid=<%=GetInt("corpid") %>">
            追踪超时率</div>
...
$.getJSON('?json=1&corpid=<%=GetInt("corpid") %>&active=<%=GetString("active") == "" ? "state" : GetString("active") %>&r='+Math.random(),p,function(data){
Core.Highcharts.loader('<%=GetString("active") == "" ? "state" : GetString("active") %>',data);
});
//return Core.submit();
}
        ...
        Core.Highcharts.loader = function (method, params) {
            ...
            switch (method) {
                case 'state':
                    ...
                    try {
                        var w = '<tr class="title"><td>姓名</td>';
                        for (layer in params['layer']) {
                            series.push({ name: params['ilayer'][L], data: params['layer:' + layer], dataLabels: { enabled: true} });
                            w += '<td>' + params['ilayer'][L] + '</td>';
                            L++;
                        }
                        w += '</tr>';
                        Core.Highcharts.box["table"] += '<tr class="title"><td colspan="' + (L + 1) + '"><span class="export"><a>导出Excel</a></span>客户状态</td></tr>' + w;

                        for (key in params['user']) {
                            Core.Highcharts.box["table"] += '<tr><td>' + params['user'][key] + '</td>';
                            for (ikey in params) {
                                if (ikey.indexOf('layer:') == -1) continue;
                                Core.Highcharts.box["table"] += '<td>' + params[ikey][key] + '</td>';
                            }
                            Core.Highcharts.box["table"] += '</tr>';
                            Core.Highcharts.box["L"]++;
                        }
                        Core.Highcharts.box["table"] += '<tr><td>合计</td>';
                        for (ikey in params['heji']) {
                            if (ikey.indexOf('layer:') == -1) continue;
                            Core.Highcharts.box["table"] += '<td>' + params['heji'][ikey] + '</td>';
                        }
                        Core.Highcharts.box["table"] += '</tr>';
                        Core.Highcharts.box["height"] = Core.Highcharts.box["bar"]["H"] + (Core.Highcharts.box["L"] * Core.Highcharts.box["bar"]["L1"]);

                        $('#container').highcharts({
                            chart: { type: 'bar', height: Core.Highcharts.box["height"] }, title: { text: '' },
                            xAxis: { categories: params['user'] },
                            yAxis: { min: 0, title: { text: ''} },
                            legend: { style: { color: '#262626', fontSize: '12px', fontWeight: 'normal'} },
                            plotOptions: { bar: { pointPadding: 1, groupPadding: 1, borderWidth: 0, pointWidth: 15, minPointLength: 2, shadow: false, dataLabels: { enabled: true} }, series: { stacking: 'normal'} },
                            series: series
                        });
                        $(table).append(Core.Highcharts.box["table"]);
                    } catch (e) { }
                    break; case 'customerlevel':
                    if (Core.of(params['user']) == 'undefined') return Core.Highcharts.isnull();
                    try {
                        var w = '<tr class="title"><td>姓名</td>';
                        for (layer in params['layer']) {
                        ...
                        w += '</tr>';
                        Core.Highcharts.box["table"] += '<tr class="title"><td colspan="' + (L + 1) + '"><span class="export"><a>导出Excel</a></span>客户级别</td></tr>' + w;
                    ...
                    try {
                        ...
                        w += '</tr>';
                        Core.Highcharts.box["table"] += '<tr class="title"><td colspan="' + (L + 1) + '"><span class="export"><a>导出Excel</a></span>追踪超时率</td></tr>' + w;
