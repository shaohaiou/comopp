<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="finality.aspx.cs" Inherits="ComOpp.BackAdmin.analyse.finality" %>
...
<body>
    <div class="Tab">
        <a href="archive.aspx?corpid=<%=GetInt("corpid") %>">销售线索分析</a> <a href="invite.aspx?corpid=<%=GetInt("corpid") %>">
            客户邀约分析</a> <a href="finality.aspx?corpid=<%=GetInt("corpid") %>" class="selected">成交与战败分析</a>
        <a href="follow.aspx?corpid=<%=GetInt("corpid") %>">客户追踪分析</a>
    </div>
    <form id="Form1" runat="server">
    <div class="nav_tabs" style="padding-left: 15px; margin-top: 5px; padding-bottom: 5px;
        border-bottom: 1px #ddd solid">
        <div class="eachTab<%if(GetString("active") == "turnover"){ %> tchose<%} %>" data-key="turnover"
            data-link="finality.aspx?active=turnover&corpid=<%=GetInt("corpid") %>">
            成交量</div>
        <div class="eachTab<%if(GetString("active") == "turnoverrate"){ %> tchose<%} %>"
            data-key="turnoverrate" data-link="finality.aspx?active=turnoverrate&corpid=<%=GetInt("corpid") %>">
            到店成交率</div>
        <div class="eachTab<%if(GetString("active") == "purgerate"){ %> tchose<%} %>" data-key="purgerate"
            style="width: 100px" data-link="finality.aspx?active=purgerate&corpid=<%=GetInt("corpid") %>">
            意向客户成交率</div>
        <div class="eachTab<%if(GetString("active") == "defeatrate"){ %> tchose<%} %>" data-key="defeatrate"
            data-link="finality.aspx?active=defeatrate&corpid=<%=GetInt("corpid") %>">
            战败率</div>
        <div class="eachTab<%if(GetString("active") == "defeatreason"){ %> tchose<%} %>"
            data-key="defeatreason" data-link="finality.aspx?active=defeatreason&corpid=<%=GetInt("corpid") %>">
            战败原因</div>
...
$.getJSON('?json=1&corpid=<%=GetInt("corpid") %>&active=<%=GetString("active") == "" ? "turnover" : GetString("active") %>&r='+Math.random(),p,function(data){
Core.Highcharts.loader('<%=GetString("active") == "" ? "turnover" : GetString("active") %>',data);
});
//return Core.submit();
}
        ...
        Core.Highcharts.loader = function (method, params) {
            ...
            switch (method) {
                case 'turnover':
                    ...
                    try {
                        series = [
{ type: 'column', name: '成交量', data: params["total"], dataLabels: { enabled: true} },
];
                        Core.Highcharts.box["table"] = '<tr class="title"><td colspan="2"><span class="export"><a>导出Excel</a></span></td></tr>';
                        Core.Highcharts.box["table"] += '<tr class="title"><td>姓名</td><td>成交量</td></tr>';
                    ...
                    break; case 'turnoverrate':
                    if (Core.of(params['user']) == 'undefined') return Core.Highcharts.isnull();
                    Core.Highcharts.addParams("columns", params["user"]);
                    try {
                        series = [
{ type: 'column', name: '到店成交率', data: params["pct"], dataLabels: { enabled: true, formatter: function () { return '' + this.y + '% '; } } },
{ type: 'column', name: '邀约到店客户量', data: params["arrive"], dataLabels: { enabled: true} },
{ type: 'column', name: '成交量', data: params["turnover"], dataLabels: { enabled: true} }
];
                        Core.Highcharts.box["table"] += '<tr class="title"><td>姓名</td><td>成交量</td><td>邀约到店客户量</td><td>到店成交率</td></tr>';
                        for (key in params['user']) {
                            Core.Highcharts.box["table"] += '<tr><td>' + params["user"][key] + '</td><td>' + params["turnover"][key] + '</td><td>' + params["arrive"][key] + '</td><td>' + params["pct"][key] + '%</td></tr>';
                            Core.Highcharts.box["L"]++;
                        }
                        Core.Highcharts.box["table"] += '<tr><td>合计</td><td>' + params["heji"]["turnover"] + '</td><td>' + params["heji"]["arrive"] + '</td><td>' + params["heji"]["pct"] + '%</td></tr>';
                        $(table).append(Core.Highcharts.box["table"]);
                    } catch (e) { }
                    Core.Highcharts.addParams("series", series);
                    Core.Highcharts.box["height"] = Core.Highcharts.box["bar"]["H"] + (Core.Highcharts.box["L"] * Core.Highcharts.box["bar"]["L" + series.length]);
                    Core.Highcharts.chart();
                    break; case 'purgerate':
                    if (Core.of(params['user']) == 'undefined') return Core.Highcharts.isnull();
                    Core.Highcharts.addParams("columns", params["user"]);
                    try {
                        series = [
{ type: 'column', name: '意向客户成交率', data: params["pct"], dataLabels: { enabled: true, formatter: function () { return '' + this.y + '% '; } } },
{ type: 'column', name: '意向客户', data: params["purge"], dataLabels: { enabled: true} },
{ type: 'column', name: '成交量', data: params["turnover"], dataLabels: { enabled: true} }
];
                        Core.Highcharts.box["table"] += '<tr class="title"><td>姓名</td><td>成交量</td><td>意向客户</td><td>意向客户成交率</td></tr>';
                        for (key in params['user']) {
                            Core.Highcharts.box["table"] += '<tr><td>' + params["user"][key] + '</td><td>' + params["turnover"][key] + '</td><td>' + params["purge"][key] + '</td><td>' + params["pct"][key] + '%</td></tr>';
                            Core.Highcharts.box["L"]++;
                        }
                        Core.Highcharts.box["table"] += '<tr><td>合计</td><td>' + params["heji"]["turnover"] + '</td><td>' + params["heji"]["purge"] + '</td><td>' + params["heji"]["pct"] + '%</td></tr>';
                        $(table).append(Core.Highcharts.box["table"]);
                    } catch (e) { }
                    Core.Highcharts.addParams("series", series);
                    Core.Highcharts.box["height"] = Core.Highcharts.box["bar"]["H"] + (Core.Highcharts.box["L"] * Core.Highcharts.box["bar"]["L" + series.length]);
                    Core.Highcharts.chart();
                    break; case 'defeatrate':
                    if (Core.of(params['user']) == 'undefined') return Core.Highcharts.isnull();
                    Core.Highcharts.addParams("columns", params["user"]);
                    try {
                        series = [
{ type: 'column', name: '战败率', data: params["pct"], dataLabels: { enabled: true, formatter: function () { return '' + this.y + '% '; } } },
{ type: 'column', name: '意向客户', data: params["purge"], dataLabels: { enabled: true} },
{ type: 'column', name: '战败数量', data: params["failure"], dataLabels: { enabled: true} }
];
                        Core.Highcharts.box["table"] += '<tr class="title"><td>姓名</td><td>战败数量</td><td>意向客户</td><td>战败率</td></tr>';
                        for (key in params['user']) {
                            Core.Highcharts.box["table"] += '<tr><td>' + params["user"][key] + '</td><td>' + params["failure"][key] + '</td><td>' + params["purge"][key] + '</td><td>' + params["pct"][key] + '%</td></tr>';
                            Core.Highcharts.box["L"]++;
                        }
                        Core.Highcharts.box["table"] += '<tr><td>合计</td><td>' + params["heji"]["failure"] + '</td><td>' + params["heji"]["purge"] + '</td><td>' + params["heji"]["pct"] + '%</td></tr>';
                        $(table).append(Core.Highcharts.box["table"]);
                    } catch (e) { }
                    Core.Highcharts.addParams("series", series);
                    Core.Highcharts.box["height"] = Core.Highcharts.box["bar"]["H"] + (Core.Highcharts.box["L"] * Core.Highcharts.box["bar"]["L" + series.length]);
                    Core.Highcharts.chart();
                    break; case 'defeatreason':
                    Core.Highcharts.box["unit"] = '';
                    if (Core.of(params['giveupcause']) == 'undefined') return Core.Highcharts.isnull();
                    Core.Highcharts.addParams("columns", params["giveupcause"]);
                    try {
                        series = [
{ type: 'column', name: '战败原因', data: params["total"], dataLabels: { enabled: true} },
];
                        Core.Highcharts.box["table"] += '<tr class="title"><td>战败原因</td><td>战败量</td></tr>';
                        for (key in params['giveupcause']) {
                            Core.Highcharts.box["table"] += '<tr><td>' + params["giveupcause"][key] + '</td><td>' + params["total"][key] + '</td></tr>';
                            Core.Highcharts.box["L"]++;
                        }
                        Core.Highcharts.box["table"] += '<tr><td>合计</td><td>' + params["heji"] + '</td></tr>';
                        $(table).append(Core.Highcharts.box["table"]);
