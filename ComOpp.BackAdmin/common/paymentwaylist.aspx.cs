...
{
    public partial class paymentwaylist : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<PaymentWayInfo> list = PaymentWays.Instance.GetListByCorpid(corpid, true);

                int total = list.Count;
                int maxpage = list.Count / rows + (list.Count % rows == 0 ? 0 : 1);

                list = list.Skip((page - 1) * rows).Take(rows).ToList<PaymentWayInfo>();
