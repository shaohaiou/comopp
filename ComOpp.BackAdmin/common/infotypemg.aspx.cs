...
{
    public partial class infotypemg : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(action))
            {
                ...
                int corpid = 0;
                if (id > 0)
                    corpid = InfoTypes.Instance.GetModel(id, true).CorporationID;
                if (GetString("action") == "del" && !string.IsNullOrEmpty(GetString("ids")))
                    InfoTypes.Instance.Delete(GetString("ids"));
                else if (GetString("action") == "moveup" && id > 0)
                    InfoTypes.Instance.MoveUp(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movedown" && id > 0)
                    InfoTypes.Instance.MoveDown(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movetop" && id > 0)
                    InfoTypes.Instance.MoveTop(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
            ...
            if (!IsPostBack)
            {
                ...
                Response.Redirect("infotypemg.aspx?corpid=" + ddlCorporationSearch.SelectedValue);
        }
    }
}