<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="paymentwaymg.aspx.cs" Inherits="ComOpp.BackAdmin.common.paymentwaymg" %>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>支付方式管理-红旭集团商机管理系统V1.0</title>
...
<body>
    <div class="Tab">
        <a href="basesetting.aspx">基础设置</a> <a href="customerlevelmg.aspx">客户等级</a> <a href="infotypemg.aspx">
            信息类型</a> <a href="infosourcemg.aspx">信息来源</a> <a href="connectwaymg.aspx">
                追踪方式</a> <a href="giveupcausemg.aspx">放弃原因</a> <a href="paymentwaymg.aspx" class="selected">支付方式</a>
    ...
    <script type="text/javascript">
        ...
        Core.Easyui.Action = function (method, o) {
            ...
            switch (method) {
                case 'add':
                    dialog('580', '300', '添加', "paymentwayedit.aspx?from=<%=CurrentUrl %>&corpid=" + corpid + "&r=" + Math.random());
                    break;
                case 'edit':
                    ...
                    dialog('580', '300', '编辑', "paymentwayedit.aspx?from=<%=CurrentUrl %>&id=" + rows[0].id + "&r=" + Math.random());
                    break;
                case 'remove':
                    var ids = []
                    for (var i = 0; i < rows.length; i++) {
                        if(rows[i].datalevel != 0)<%if (!Admin.Administrator){%>if(rows[i].datalevel != 0)<%} %>
                            ids.push(rows[i].id);
                    }
        ...
        $(document).ready(function () {
            Core.Easyui.get("/common/paymentwaylist.aspx?corpid=" + corpid);
        })
    </script>
    <script type="text/javascript">
        $(document).ready(function () {
