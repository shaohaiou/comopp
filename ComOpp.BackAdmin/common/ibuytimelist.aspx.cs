...
{
    public partial class ibuytimelist : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<IbuytimeInfo> list = Ibuytimes.Instance.GetListByCorpid(corpid, true);

                int total = list.Count;
                int maxpage = list.Count / rows + (list.Count % rows == 0 ? 0 : 1);

                list = list.Skip((page - 1) * rows).Take(rows).ToList<IbuytimeInfo>();
