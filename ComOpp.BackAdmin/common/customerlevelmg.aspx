<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="customerlevelmg.aspx.cs"
    Inherits="ComOpp.BackAdmin.common.customerlevelmg" %>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>客户等级管理-红旭集团销售客户管理系统V1.0</title>
...
<body>
    <div class="Tab">
        <a href="basesetting.aspx">基础设置</a> <a href="customerlevelmg.aspx?corpid=<%=GetInt("corpid") %>" class="selected">
            客户等级</a> <a href="infotypemg.aspx?corpid=<%=GetInt("corpid") %>">信息类型</a> <a href="infosourcemg.aspx?corpid=<%=GetInt("corpid") %>">信息来源</a>
        <a href="connectwaymg.aspx?corpid=<%=GetInt("corpid") %>">追踪方式</a> <a href="giveupcausemg.aspx?corpid=<%=GetInt("corpid") %>">放弃原因</a> <a href="paymentwaymg.aspx?corpid=<%=GetInt("corpid") %>">
            支付方式</a> <a href="ibuytimemg.aspx?corpid=<%=GetInt("corpid") %>">拟购时间</a> <a href="tracktagmg.aspx?corpid=<%=GetInt("corpid") %>">线索标签</a>
        <a href="archivesetting.aspx?corpid=<%=GetInt("corpid") %>">转出设置</a>
    </div>
    <div>
    ...
    <script type="text/javascript">
        ...
        { field: 'drtday', title: '降级天数',width:80, align: 'center' },
        { field: 'alarmday', title: '报警天数',width:80, align: 'center'},
        ...
        Core.Easyui.Action = function (method, o) {
            ...
            switch (method) {
                case 'add':
                    dialog('580', '300', '添加', "customerleveledit.aspx?from=<%=CurrentUrl %>&corpid=" + corpid + "&r=" + Math.random());
                    break;
                case 'edit':
                    if (!rows.length) { $.messager.alert('提示', '请选择一条记录', 'info'); return; }
                    if (rows.length > 1) { $.messager.alert('提示', '该操作仅允许1条数据', 'info'); return; }
                    dialog('580', '300', '编辑', "customerleveledit.aspx?from=<%=CurrentUrl %>&id=" + rows[0].id + "&r=" + Math.random());
                    break;
                case 'remove':
                    var ids = []
        ...
        $(document).ready(function () {
            Core.Easyui.get("/common/customerlevellist.aspx?corpid=" + corpid);
        })
    </script>
    <script type="text/javascript">
        $(document).ready(function () {
