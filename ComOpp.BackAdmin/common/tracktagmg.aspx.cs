...
{
    public partial class tracktagmg : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(action))
            {
                ...
                int corpid = 0;
                if (id > 0)
                    corpid = Tracktags.Instance.GetModel(id, true).CorporationID;
                if (GetString("action") == "del" && !string.IsNullOrEmpty(GetString("ids")))
                    Tracktags.Instance.Delete(GetString("ids"));
                else if (GetString("action") == "moveup" && id > 0)
                    Tracktags.Instance.MoveUp(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movedown" && id > 0)
                    Tracktags.Instance.MoveDown(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movetop" && id > 0)
                    Tracktags.Instance.MoveTop(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
            ...
            if (!IsPostBack)
            {
                ...
                Response.Redirect("tracktagmg.aspx?corpid=" + ddlCorporationSearch.SelectedValue);
        }
    }
}