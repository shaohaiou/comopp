<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="connectwaymg.aspx.cs" Inherits="ComOpp.BackAdmin.common.connectwaymg" %>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>追踪方式管理-红旭集团销售客户管理系统V1.0</title>
...
<body>
    <div class="Tab">
        <a href="basesetting.aspx">基础设置</a> <a href="customerlevelmg.aspx?corpid=<%=GetInt("corpid") %>">客户等级</a> <a href="infotypemg.aspx?corpid=<%=GetInt("corpid") %>">
            信息类型</a> <a href="infosourcemg.aspx?corpid=<%=GetInt("corpid") %>">信息来源</a> <a href="connectwaymg.aspx?corpid=<%=GetInt("corpid") %>" class="selected">
    ...
    <script type="text/javascript">
        ...
        Core.Easyui.Action = function (method, o) {
            ...
            switch (method) {
                case 'add':
                    dialog('580', '300', '添加', "connectwayedit.aspx?from=<%=CurrentUrl %>&corpid=" + corpid + "&r=" + Math.random());
                    break;
                case 'edit':
                    ...
                    dialog('580', '300', '编辑', "connectwayedit.aspx?from=<%=CurrentUrl %>&id=" + rows[0].id + "&r=" + Math.random());
                    break;
                case 'remove':
                    var ids = []
        ...
        $(document).ready(function () {
            Core.Easyui.get("/common/connectwaylist.aspx?corpid=" + corpid);
        })
    </script>
    <script type="text/javascript">
        $(document).ready(function () {
