...
{
    public partial class infosourcelist : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                List<InfoSourceInfo> list = InfoSources.Instance.GetListByCorpid(corpid, true);

                int total = list.Count;
                int maxpage = list.Count / rows + (list.Count % rows == 0 ? 0 : 1);

                list = list.Skip((page - 1) * rows).Take(rows).ToList<InfoSourceInfo>();
