...
{
    public partial class paymentwaymg : AdminBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(action))
            {
                ...
                int corpid = 0;
                if (id > 0)
                    corpid = PaymentWays.Instance.GetModel(id, true).CorporationID;
                if (GetString("action") == "del" && !string.IsNullOrEmpty(GetString("ids")))
                    PaymentWays.Instance.Delete(GetString("ids"));
                else if (GetString("action") == "moveup" && id > 0)
                    PaymentWays.Instance.MoveUp(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movedown" && id > 0)
                    PaymentWays.Instance.MoveDown(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
                else if (GetString("action") == "movetop" && id > 0)
                    PaymentWays.Instance.MoveTop(id, " WHERE [CorporationID] = " + corpid + " AND State = 1");
            ...
            if (!IsPostBack)
            {
                ...
                Response.Redirect("paymentwaymg.aspx?corpid=" + ddlCorporationSearch.SelectedValue);
        }
    }
}